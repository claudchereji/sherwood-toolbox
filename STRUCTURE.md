...
| `toolbox/tools/reconciler/{extract,match,reconcile,report,playbook}.py` | The estimate-comparison engine (ported unchanged from the standalone reconciler). `extract.py` reads PDF text and OCR via PyMuPDF only. `report.py` renders the Markdown/CSV that now feeds the log. | Change parsing (`extract.py`), matching (`match.py`), the RCV bridge (`reconcile.py`), or the log's report/CSV body (`report.py`). |
| `toolbox/tools/reconciler/markup.py` | Paints the reconciliation onto the carrier PDF (PyMuPDF): a prepended summary page, in-line highlight bands + numbered margin tabs on the line items the contractor measured higher, and appended detail pages (quantity differences, missing scope, RCV build-up, hypotheses, statements). Line items are located by re-clustering page words into rows and matching the printed line number. | Change what the carrier markup looks like or how rows are located. |
